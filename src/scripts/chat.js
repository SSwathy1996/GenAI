...
class ChatUI {
    ...
    this.codeGeneratorType     = 'CUCUMBER_ONLY'; // default to feature file, will be set from storage
    ...
    initialize() {
        ...
        if (this.resetButton) {
            this.resetButton.addEventListener('click', () => {
                ...
                // Load stored keys and codeGenMode
                chrome.storage.sync.get(
                    ['groqApiKey','openaiApiKey','testleafApiKey','selectedModel','selectedProvider','codeGenMode'],
                    (result) => {
                        ...
                        // Set code generator type based on mode
                        if (result.codeGenMode === 'PAGE') {
                            this.codeGeneratorType = 'SELENIUM_JAVA_PAGE_ONLY';
                        } else if (result.codeGenMode === 'TEST_DATA') {
                            this.codeGeneratorType = 'TEST_DATA_ONLY';
                        } else if (result.codeGenMode === 'FEATURE') {
                            this.codeGeneratorType = 'CUCUMBER_ONLY';
                        }
                });
                ...
                chrome.storage.onChanged.addListener((changes) => {
                        ...
                        if (changes.codeGenMode) {
                            if (changes.codeGenMode.newValue === 'PAGE') {
                                this.codeGeneratorType = 'SELENIUM_JAVA_PAGE_ONLY';
                            } else if (changes.codeGenMode.newValue === 'TEST_DATA') {
                                this.codeGeneratorType = 'TEST_DATA_ONLY';
                            } else if (changes.codeGenMode.newValue === 'FEATURE') {
                                this.codeGeneratorType = 'CUCUMBER_ONLY';
                            }
                        }
                });
    ...
    async sendMessage() {
        ...
        // Use only the selected codeGeneratorType as the prompt key
        let promptKeys = [];
        if (this.codeGeneratorType === 'CUCUMBER_ONLY') {
            promptKeys = ['CUCUMBER_ONLY'];
        } else if (this.codeGeneratorType === 'SELENIUM_JAVA_PAGE_ONLY') {
            promptKeys = ['SELENIUM_JAVA_PAGE_ONLY'];
        } else if (this.codeGeneratorType === 'TEST_DATA_ONLY') {
            promptKeys = ['TEST_DATA_ONLY'];
        }
        if (promptKeys.length === 0) {
            this.addMessage('Please select a generation mode (Feature File, Page Class, or Test Data Generator).', 'system');
            return;
        }
