...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    // Checkbox logic for code generation mode
    ...
    function updateCheckboxes(changed) {
      if (changed === 'FEATURE') {
        featureCheckbox.checked = true;
        pageCheckbox.checked = false;
        testDataCheckbox.checked = false;
        storage.set({ codeGenMode: 'FEATURE' });
      } else if (changed === 'PAGE') {
        featureCheckbox.checked = false;
        pageCheckbox.checked = true;
        testDataCheckbox.checked = false;
        storage.set({ codeGenMode: 'PAGE' });
      } else if (changed === 'TEST_DATA') {
        featureCheckbox.checked = false;
        pageCheckbox.checked = false;
        testDataCheckbox.checked = true;
        storage.set({ codeGenMode: 'TEST_DATA' });
      }
    }
    if (featureCheckbox) featureCheckbox.addEventListener('change', () => { if (featureCheckbox.checked) updateCheckboxes('FEATURE'); });
    if (pageCheckbox) pageCheckbox.addEventListener('change', () => { if (pageCheckbox.checked) updateCheckboxes('PAGE'); });
    if (testDataCheckbox) testDataCheckbox.addEventListener('change', () => { if (testDataCheckbox.checked) updateCheckboxes('TEST_DATA'); });

    // Set default checked state on load
    storage.get(['codeGenMode'], (result) => {
      if (result.codeGenMode === 'PAGE') updateCheckboxes('PAGE');
      else if (result.codeGenMode === 'TEST_DATA') updateCheckboxes('TEST_DATA');
      else updateCheckboxes('FEATURE');
    });
